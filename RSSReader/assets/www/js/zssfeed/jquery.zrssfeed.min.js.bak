...
(function(h) {
    h.fn.rssfeed = function(q, f, r) {
        f = h.extend({
            ...
            linktarget: "_self",
			status:"00",
			currentDATE: new Date(), //currentDATE: new Date().toString("yyyy-M-dd hh:mm:ss"))
			latestDATE:new Date()
        },
        f);
        return this.each(function(s, l) {
            ...
            function(b) {
                if (200 == b.responseStatus) {
                    ...
                    if (g) {
                        ...
                        for (e = 0; e < g.entries.length; e++) {
							...
							pubdate = Date._parse(c.publishedDate);
                            //latestpubdate = Date.parse(g.entries[0].publishedDate);
                            //currentdate = Date.parse(f.currentDATE); //-24*60*60*1000;
                            formatpubdate = new Date(pubdate);//.toString("yyyy-M-dd hh:mm:ss"));2006-10-17 00:20:00

							console.log(c.publishedDate); //-0700
							console.log(new Date(c.publishedDate)); //GMT
							console.log(pubdate); //ms
							console.log(formatpubdate); 
							console.log(f.latestDATE);c
							console.log("#####");
							...
							//alert(f.status);//alert(f.latestDATE);
							//alert("美国时间："+c.publishedDate+"中国时间"+formatpubdate);
							//alert(new Date(pubdate)+"    "+Date._parse(f.latestDATE)); //pubdate在下午会出现+12的情况
							i = new Date(c.publishedDate);//alert(i.toLocaleDateString()+" "+i.toLocaleTimeString());
							if(f.status=="01")
							{
								alert("非初次加载");
							    if(pubdate>Date._parse(f.latestDATE))
								{
									alert("有更新");
								    titleArr[e] = title;
								    linkArr[e] = link;
                ...
                } else f.showerror && (g = "" != f.errormsg ? f.errormsg: b.responseDetails),
                h(l).html('<div class="rssError"><p>' + g + "</p></div>")
            })
        })
    }
})(jQuery);